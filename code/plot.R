...
plot.new()
with(res, plot(log2FoldChange, -log10(padj), pch=20, xaxt="n",
...
genes1 <- union(subset(table1, Category=="Chemotaxis", select=ID)$ID, 
                subset(table2, Category=="Chemotaxis", select=ID)$ID)
genes2 <- union(subset(table1, Category=="Metal resistance", select=ID)$ID,
                subset(table2, Category=="Metal resistance", select=ID)$ID)
genes3 <- union(subset(table1, Category=="Motility", select=ID)$ID,
                subset(table2, Category=="Motility", select=ID)$ID)
genes4 <- union(subset(table1, Category=="Polysaccharide", select=ID)$ID,
...
legend("top", legend=c("Chemotaxis", "Metal resistance", "Motility", "Polysaccharide"),
...
abline(v=2.5, col="black", lty=2)
abline(h=-log10(0.05), col="black", lty=3)
genes5 <- union(subset(table1, Category=="ATP synthase", select=ID)$ID,
                subset(table2, Category=="ATP synthase", select=ID)$ID)
genes6 <- union(subset(table1, Category=="Carbon fixation", select=ID)$ID,
                subset(table2, Category=="Carbon fixation", select=ID)$ID)
genes7 <- union(subset(table1, Category=="Oxidative stress response", select=ID)$ID,
...
legend("top", legend=c("ATP synthase", "Carbon fixation", "Oxidative stress response"),
